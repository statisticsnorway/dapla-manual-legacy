...
```
Et eksempel med lesing av stedfestede VoF situasjonsfiler kan dermed se slik ut:
```python
import pandas as pd
import geopandas as gpd
import dapla as dp
...
sted_vof = dp.read_pandas(gcs_path= f"{VOFSTI}/stedfesting-situasjonsuttak_p{AAR}-01_v1.parquet",
               file_format = "parquet")
vof_vof = gpd.GeoDataFrame(sted_vof, geometry=gpd.points_from_xy(sted_vof.y_koordinat, sted_vof.x_koordinat))
vof_vof = vof_vof.set_crs('epsg:25833')
```
