...
1. project_id(prosjekt id) for kilden, den finner man ved å følge beskrivelsen [her](./gcc-avansert.qmd#velg-prosjekt). 
1. folder_prefix beskriver stien til filene som skal behandles og fungerer på samme måte som i [config.yaml](./kildedata-prosessering.qmd#innhold-i-config.yaml-for-boller)
1. source_name finner man ved å se på navnet til mappen hvor kilden konfigureres, i eksempelet [her](./kildedata-prosessering.qmd#struktur-i-iac-repo) ser vi at team `smaabakst` har to kilder `boller` og `rundstykker`.  
